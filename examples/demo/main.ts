...
import type { MemoryEngine } from '../../src/engine';
import type { SearchEnvelope } from '../../src/types';
...
async function chapter4(engine: MemoryEngine): Promise<void> {
  ...
  const tm = envelope.tagMemo && typeof envelope.tagMemo === 'object'
    ? envelope.tagMemo as { rankedTags?: unknown[] }
    : {};
  if (tm.rankedTags && tm.rankedTags.length) {
    const tags = tm.rankedTags.slice(0, 5).map((t: unknown) => {
  ...
  const epa = envelope.epa && typeof envelope.epa === 'object'
    ? envelope.epa as { ready?: boolean; queryAnalysis?: { logicDepth?: number; dominantAxes?: unknown[]; resonance?: { resonance?: number } } }
    : {};
  ...
  const features = pyr?.features && typeof pyr.features === 'object'
    ? pyr.features as { depth?: number; coverage?: number; novelty?: number }
    : undefined;
  if (features && features.depth != null && features.coverage != null && features.novelty != null) {
    console.log(`    ${C.dim}残差金字塔: 深度 ${features.depth}｜覆盖率 ${(features.coverage * 100).toFixed(1)}%｜新颖度 ${features.novelty.toFixed(2)}${C.reset}`);
  }
}
