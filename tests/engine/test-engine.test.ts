...
import { createMemoryEngine, MemoryEngine, DEFAULT_CONFIG, mergeConfig, loadRagParams, loadRagParamsSync } from '../../index';
import type {
 ...
 */
function makeFakeEmbeddingProvider(dim = DIM): EmbeddingProviderContract & { name: string } {
  return {
...
test('initialize() is idempotent and exposes ragParams', async () => {
  ...
  assert.strictEqual(
    ((engine.ragParams.KnowledgeBaseManager as UnknownRecord).resultDeduplication as UnknownRecord).semanticThreshold,
    0.83
  );
...
test('injected fake embedding provider is used instead of the network provider', async () => {
  ...
  assert.ok(
    !(engine.ctx.embeddingProvider as EmbeddingProviderContract & { apiUrl?: string }).apiUrl,
