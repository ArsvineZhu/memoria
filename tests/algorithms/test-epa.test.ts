...
test("EPA.project computes logic depth from orthogonal basis", () => {
  ...
  assert.ok(
    focusedResult.logicDepth > 0.5,
    "focused vector should have high logic depth",
  );

  const spread = new Float32Array([1, 1, 0, 0]);
  const spreadResult = epa.project(spread);
  assert.ok(
    spreadResult.logicDepth < focusedResult.logicDepth,
    "spread vector should have lower logic depth",
  );
});
...
test("EPA.computeBasis builds basis from tag vectors", () => {
  ...
  assert.strictEqual(at(basis.orthoBasis, 0, "EPA basis").length, dim);
});
