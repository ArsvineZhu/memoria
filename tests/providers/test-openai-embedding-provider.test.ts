...
import OpenAIEmbeddingProvider = require('../../src/providers/openai-embedding-provider');
...
test('embedBatch returns nulls for all-oversize texts without calling fetch', async () => {
  ...
  global.fetch = (async (
    ...
    _opts?: RequestInit,
  ) => { fetchCalled = true; }) as unknown as typeof fetch;
...
test('embedBatch handles 429 by switching to fallback model', async () => {
  ...
  global.fetch = (async (
    ...
    callCount++;
    const body = JSON.parse(String(opts?.body || '{}')) as { model: string; input: string[] };
