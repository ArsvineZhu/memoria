...
import type { EmbeddingProviderContract, QueryVector } from '../../src/types';

import QueryEmbedderStage = require('../../src/stages/retrieval/query-embedder');
import VectorSearcherStage = require('../../src/stages/retrieval/vector-searcher');
import BM25SearcherStage = require('../../src/stages/retrieval/bm25-searcher');
...
const fakeEmbeddingProvider: EmbeddingProviderContract = {
  ...
  embedBatch: async (texts = []) => texts.map((t: string) => {
...
test('QueryEmbedderStage supports injectable rephraserFn query expansion', async () => {
  const stage = new QueryEmbedderStage();
  const rephraserFn = async (query: string, i: number): Promise<string> => `${query} 重述${i}`;
...
test('QueryEmbedderStage masks small vector components when epsilon is configured', async () => {
  ...
  const provider = {
    getDimension: () => dim,
    embedBatch: async (texts: readonly string[]) => texts.map(() => [1, 0.5, -0.2, 0.05])
  } satisfies EmbeddingProviderContract;
  const ctx = new PipelineContext({
...
test('BM25SearcherStage accepts a config tokenizer hook', async () => {
  ...
  const tokenizer = (text: string): string[] => String(text).split(/[\s,]+/).filter(Boolean);
