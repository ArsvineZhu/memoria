...
function loadApiKey(): string | null {
  ...
  const line = fs
    ...
    .split(/\r?\n/)
    .find((l: string) => /^EMBED_API_KEY\s*=/.test(l.trim()));
  if (!line) return null;
  return line
    .split("=")
    .slice(1)
...
const QUERIES = [
  ...
  {
    tag: "同义改写",
    query: "晚上总是睡不着，有什么助眠的办法",
    hint: "→ 中医养生（“泡脚按揉涌泉穴”原文）",
  },
  {
    tag: "同义改写",
    query: "想减肥又不想去健身房，怎么安排运动",
    hint: "→ 健身计划（“低冲击运动保护膝盖”）",
  },
...
async function main() {
  ...
  try {
    ...
    for (const name of fs
      .readdirSync(path.join(__dirname, "../../tests/fixtures/real-docs"))
      ...
      fs.copyFileSync(
        path.join(__dirname, "../../tests/fixtures/real-docs", name),
        target,
      );
    ...
    console.log(
      `◈ 已入库 ${stats.files} 个文件 / ${stats.chunks} 个 chunk / ${stats.vectorStats.totalVectors} 个向量\n`,
    );
    ...
    for (const { tag, query, hint } of QUERIES) {
      ...
      console.log(`│  ${hint}`);
      const out: SearchEnvelope = await kb.search(query);
      ...
      const rank = ["①", "②", "③"];
      top.forEach((r, i) => {
        const source = String(r.path || r.fullPath || "?")
          .split(/[\\/]/)
          .pop();
