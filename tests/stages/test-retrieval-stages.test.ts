...
test("VectorSearcherStage expands tag hits to chunks of tagged files", async () => {
  ...
  const chunkIds2 = await metaStore.insertChunks(f2, [
    { chunkIndex: 0, content: "untagged b" },
  ]);
  const [tag7, tag8] = await metaStore.upsertTags([
    { name: "重要", vector: null },
    { name: "普通", vector: null },
  ]);
  ...
  assert.deepStrictEqual(
    ids,
    [...chunkIds1].sort((a, b) => a - b),
  );
  for (const cid of chunkIds2)
...
async function seedBm25Corpus() {
  ...
  const [c3] = await store.insertChunks(f3, [
    { chunkIndex: 0, content: "delta epsilon" },
  ]);
...
test("CandidateMergerStage honors configurable vector/bm25 weights", async () => {
  ...
  const vecCtx = new PipelineContext({
    config: { vectorWeight: 0.9, bm25Weight: 0.1 },
  });
...
test("full retrieval pipeline: query -> embed -> vector + bm25 -> merged", async () => {
  ...
  const [cA] = await metaStore.insertChunks(fA, [
    { chunkIndex: 0, content: "猫的记忆" },
  ]);
  const [cB] = await metaStore.insertChunks(fB, [
