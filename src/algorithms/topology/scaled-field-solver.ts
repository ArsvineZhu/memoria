...
export interface SolverOptions extends UnknownRecord {
  ...
  localOperator?: FieldOperator;
  transferOperator?: FieldOperator;
  dualOperator?: DualOperator;
  sourceField: Map<number, number> | readonly (readonly unknown[])[] | Float64Array;
...
export interface ScaledFieldDiagnostics extends UnknownRecord {
  iterations: number;
  converged: boolean;
...
export interface ScaledFieldResult {
  ...
  localDomain: SupportDomain;
  transferDomain: SupportDomain;
  diagnostics: ScaledFieldDiagnostics;
}
