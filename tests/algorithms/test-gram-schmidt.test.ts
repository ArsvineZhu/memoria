...
} from "../../src/algorithms/gram-schmidt.js";
...
test("orthogonalize produces orthogonal basis (Modified Gram-Schmidt)", () => {
  ...
  for (let i = 0; i < basis.length; i++) {
    for (let j = i + 1; j < basis.length; j++) {
      const dot = dotProduct(at(basis, i, "basis"), at(basis, j, "basis"));
      assert.ok(
        Math.abs(dot) < 1e-5,
        `basis ${i} and ${j} should be orthogonal, dot=${dot}`,
      );
    }
  }
...
test("orthogonalize handles linearly dependent vectors", () => {
  ...
  assert.strictEqual(at(basisCoefficients, 1, "basis coefficients"), 0); // second has no contribution
});

test("orthogonalProjection projects vector onto subspace and returns residual", () => {
  const dim = 3;
  const vector = new Float32Array([1, 1, 1]);
  const tags = [new Float32Array([1, 0, 0]), new Float32Array([0, 1, 0])];
  const { projection, residual } = orthogonalProjection(vector, tags, dim);

  // Projection should be [1, 1, 0]
  assert.ok(Math.abs(at(projection, 0, "projection") - 1) < 1e-5);
  assert.ok(Math.abs(at(projection, 1, "projection") - 1) < 1e-5);
  assert.ok(Math.abs(at(projection, 2, "projection")) < 1e-5);

  // Residual should be [0, 0, 1]
  assert.ok(Math.abs(at(residual, 0, "residual")) < 1e-5);
  assert.ok(Math.abs(at(residual, 1, "residual")) < 1e-5);
  assert.ok(Math.abs(at(residual, 2, "residual") - 1) < 1e-5);
