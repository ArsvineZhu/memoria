...
class TriviumDBAdapter {
  ...
  metadataStore: TdbStoreContract | MetadataStoreContract | null;
  indexName: string;
  dimension: number;
  _nextId: number;
  /**
  ...
  async searchHybrid(
    ...
    const byId = new Map<number, { id: number; vector: number; bm25: number }>();
  ...
  async _keywordHits(queryText: string, options: UnknownRecord): Promise<TriviumSearchHit[]> {
    ...
    const docs: Array<{ id: number; tokens: string[] }> = [];
