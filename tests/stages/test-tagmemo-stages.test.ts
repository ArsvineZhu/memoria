...
import { propagate, computeFirWeights } from "../../src/algorithms/wave-propagation.js";
import { computeRiverObservability } from "../../src/algorithms/topology/river-observability.js";
import {
  buildRowOperator,
  solveDualScaledFields,
} from "../../src/algorithms/topology/scaled-field-solver.js";

import TagMemoV9Stage from "../../src/stages/memo/tagmemo-v9.js";
import TagMemoV10Stage from "../../src/stages/memo/tagmemo-v10.js";
import RiverMemoStage from "../../src/stages/memo/rivermemo.js";
import type {
...
test("wave propagation: activations decay monotonically along a directed line", () => {
  ...
  assert.ok(
    e0 > e1 && e1 > e2 && e2 > 0,
    "energy must decay monotonically along the line",
  );
  ...
  assert.ok(
    ...
    "edge at tension 1.0 is a wormhole: decay 0.7",
  );
...
test("wave propagation: wormhole edges keep momentum and reach further nodes", () => {
  ...
  assert.ok(
    withWormhole.activations.get(3)! > 0,
...
test("TagMemoV9Stage: activates the tag graph and emits the energy field", async () => {
  ...
  assert.ok(
    out.tagMemo!.activations!.get(1)! > 0,
...
test("TagMemoV10Stage: solves dual scaled fields over the v9 wave", async () => {
  ...
  assert.ok(
    Array.isArray(out.tagMemo.sourceField) && out.tagMemo.sourceField.length > 0,
  );
...
class InMemoryKvStore implements RiverStateStore {
...
function baseRiverInput(): PipelineData {
  return {
    tagMemo: {
      version: "v9",
      riverGraph: {
...
function riverStep(store: InMemoryKvStore, config: MemoryConfigOverrides = {}) {
...
test("RiverMemoStage: river state persists across three ticks", async () => {
  ...
  assert.ok(
    Math.abs(state.flows["1-2"].flow - 0.6) < 1e-9,
    "flows accumulate across ticks",
  );
});
...
test("RiverMemoStage: confluent branches merge into the target node", async () => {
  ...
  assert.ok(
    out.riverMemo!.nodeTotals![2] >= 0.4,
...
test("RiverMemoStage: regime controls the rerank formula", async () => {
  ...
  collapsedInput.tagMemo!.riverGraph!.diagnostics = {
    seedNodes: 1,
    reachedNodes: 1,
    activeEdges: 0,
  };
  ...
  assert.strictEqual(
    collapsed.mergedCandidates![0].score,
    0.6,
  ...
  assert.ok(
    sparse.riverMemo!.regime === "sparse" || sparse.riverMemo!.regime === "dense",
  );
});
