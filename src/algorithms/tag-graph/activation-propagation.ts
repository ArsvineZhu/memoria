...
import {
  ...
  neighborsOf,
  sourceEntries,
} from "./activation-propagation-input.js";
import type {
  ActivationConfig,
...
/** Pure state-propagation kernel over an injected tag association graph. */
function propagate(options: ActivationOptions = {}): ActivationPropagationResult {
  const config: ActivationConfig = options.config || {};
  ...
  const maxPropagationStates = Math.max(
    100,
    integer(config.maxPropagationStates ?? 2000, 2000),
  );
  ...
  for (const tag of sources) {
    ...
    propagationProvenance.set(tag.id, { sourceType, hop: 0, seedId: tag.id });
  }
