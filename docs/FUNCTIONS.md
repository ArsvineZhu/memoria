...
| 2    | `TagExtractorStage`        | `tag-extractor.ts`                | 合并 `markdown`/`mdx` front matter tags 与文末连续 `Tag:` 行；黑名单 / 超集黑名单 / 长度与日期过滤 / maxTagsPerFile；与已有文件标签比较并识别 tag-only 变化                                            |
| 3    | `ChunkerStage`             | `text-chunker.ts`                 | 按句子切块（`split(/(?<=[。？！.!?\n])/)`），tiktoken 计数，超长句强制切分，相邻块按 overlapTokens 重叠                                                                                                |
| 4    | `ChunkEmbedderStage`       | `chunk-embedder.ts`               | 批量嵌入采用完整批次校验；数量不匹配、null、维度错误或非有限值均中止整个文档摄入，不提交部分 revision                                                                                                  |
...
| `ExpanderStage`           | `expander.ts`                               | 前 `expandCount` 个候选展开同文件兄弟块；开启 `fullDocumentExpansionEnabled` 时按 chunk 顺序合并父文件全文，分数 ×`expansionBoost`                                                                                                                                                                               |
| `ResultFormatterStage`    | `stages/output/result-formatter.ts`         | 终格式化：`{id, chunkId, content, path, sourceFile, fileId, diaryName, score, similarity, updatedAt, mtime, tags, matchedTags, memoScore?, source, decay?, associationChannel?, associationOf?, rerankScore?}` + `resultCount`；缺失字段从元数据回查；开启截断时对 hydration 后的正文再次执行 `maxContentLength` |
