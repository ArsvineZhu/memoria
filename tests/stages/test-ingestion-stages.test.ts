...
import FileReaderStage = require('../../src/stages/ingestion/file-reader');
import TagExtractorStage = require('../../src/stages/ingestion/tag-extractor');
import ChunkerStage = require('../../src/stages/ingestion/text-chunker');
import ChunkEmbedderStage = require('../../src/stages/ingestion/chunk-embedder');
import TagEmbedderStage = require('../../src/stages/ingestion/tag-embedder');
import type {
...
test('TagExtractorStage requires content to be a string', async () => {
  const stage = new TagExtractorStage();
  const ctx = makeCtx({});
  const missing = await stage.process({ content: 123 } as unknown as import('../../src/types').PipelineData, ctx);
...
test('ChunkEmbedderStage embeds each chunk and filters failed vectors', async () => {
  ...
  const chunkProvider: EmbeddingProviderContract = {
    getDimension: () => dim,
    embedBatch: async (texts = []) => texts.map((text: string, i: number) => (i === 1 ? null : [i, i + 1, i + 2]))
  };
...
test('TagEmbedderStage embeds each tag and filters failed vectors', async () => {
  ...
  const tagProvider: EmbeddingProviderContract = {
    getDimension: () => dim,
    embedBatch: async (texts = []) => texts.map((text: string, i: number) => (i === 0 ? null : [0.5, 0.6, 0.7]))
  };
