...
use crate::tag_graph_observation::ActivationPropagationObservation;

const RESULT_SCHEMA: &str = "propagation-structure-reranker-v1";
const ALGORITHM_VERSION: &str = "propagation-structure-reranker-v1-rust";
...
struct QueryStateInput {
    ...
    #[serde(default)]
    propagation_nodes: Vec<PropagationNode>,
    #[serde(default)]
    propagation_edges: Vec<PropagationEdge>,
    #[serde(default)]
    propagation_provenance: Vec<SourceProvenance>,
...
struct NativeConfig {
    query_k: usize,
    denoised_k: usize,
    local_distribution_k: usize,
    extended_distribution_k: usize,
    bm25_k: usize,
    anchor_k: usize,
    ...
    minimum_propagation_edge_flow: f64,
    maximum_propagation_edges: usize,
    ...
    structure_bonus_cap: f64,
    association_path_saturation: f64,
...
impl Default for NativeConfig {
    fn default() -> Self {
        Self {
            query_k: 100,
            denoised_k: 100,
            local_distribution_k: 100,
            extended_distribution_k: 100,
            bm25_k: 50,
            anchor_k: 50,
            ...
            minimum_propagation_edge_flow: 0.015,
            maximum_propagation_edges: 96,
            node_only_reliability_cap: 0.2,
            kappa_edge: 0.5,
            kappa_ratio: 0.3,
            spread_epsilon: 0.02,
            inactive_threshold: 0.12,
            ...
            structure_bonus_cap: 0.08,
            association_path_saturation: 0.15,
            ...
            spread_gamma: 1.0,
            structure_role_min_spread: 0.12,
...
pub(crate) struct TagGraphArtifact {
    ...
    pub(crate) shortcut_edges: HashSet<(i64, i64)>,
    pub(crate) provenance: HashMap<(i64, i64), Vec<(i64, f64)>>,
}

impl TagGraphArtifact {
...
///
/// 该对象只包含请求相关的小型 propagation trace 和两条查询向量，不复制全库图或 Tag 向量。
pub(crate) struct TagRetrievalObservation {
    pub(crate) artifact_sig: String,
    pub(crate) artifact_generation: u64,
    pub(crate) observation: Arc<ActivationPropagationObservation>,
    ...
    pub(crate) extended_support_ids: Arc<Vec<i64>>,
}

struct TagRetrievalQueryCacheEntry {
    value: Arc<TagRetrievalObservation>,
    inserted_at: Instant,
}

/// VexusIndex-owned canonical tag retrieval runtime.
///
/// 活动资产使用 Arc 快照：查询开始时克隆一次 Arc，后续发布不会改变本次查询；
/// 旧代资产在最后一个查询释放后自动回收。请求观测缓存与活动资产代际绑定，
/// 让 propagation support 与 PropagationStructure 直接复用统一管线产物，避免 trace 经 JS JSON 往返。
pub(crate) struct TagRetrievalRuntime {
    active_artifact: RwLock<Option<(String, u64, Arc<TagGraphArtifact>)>>,
    generation: AtomicU64,
    query_sequence: AtomicU64,
    query_cache: Mutex<HashMap<String, TagRetrievalQueryCacheEntry>>,
    query_cache_order: Mutex<VecDeque<String>>,
    query_cache_capacity: usize,
    query_cache_ttl: Duration,
}

impl TagRetrievalRuntime {
    ...
    fn get(
        &self,
        artifact_sig: &str,
    ) -> std::result::Result<Option<Arc<TagGraphArtifact>>, String> {
        let guard = self
            .active_artifact
            .read()
            .map_err(|error| format!("tag retrieval runtime read lock failed: {}", error))?;
        Ok(guard
            .as_ref()
    ...
    pub(crate) fn publish(
        &self,
        artifact_sig: &str,
        artifact: Arc<TagGraphArtifact>,
    ) -> std::result::Result<u64, String> {
        let generation = self.generation.fetch_add(1, AtomicOrdering::AcqRel) + 1;
        let mut guard = self
            .active_artifact
            .write()
            .map_err(|error| format!("tag retrieval runtime publish lock failed: {}", error))?;
    ...
    fn active_generation(&self, artifact_sig: &str) -> std::result::Result<u64, String> {
        let guard = self
            .active_artifact
            .read()
            .map_err(|error| format!("tag retrieval runtime generation lock failed: {}", error))?;
        guard
            .as_ref()
            .filter(|(signature, _, _)| signature == artifact_sig)
            ...
            .ok_or_else(|| {
                format!(
                    "tag retrieval runtime artifact {} is not the active generation",
                    artifact_sig
                )
            })
    }
    ...
    fn clear_query_cache(&self) -> std::result::Result<(), String> {
        self.query_cache
            .lock()
            .map_err(|error| format!("tag retrieval query cache lock failed: {}", error))?
            .clear();
        self.query_cache_order
            .lock()
            .map_err(|error| format!("tag retrieval query cache order lock failed: {}", error))?
            .clear();
        Ok(())
    }
    ...
    pub(crate) fn store_query_observation(
        ...
        let value = Arc::new(TagRetrievalObservation {
            ...
            extended_support_ids: Arc::new(extended_support_ids),
        });
    ...
    pub(crate) fn get_query_observation(
        ...
        handle: &str,
        artifact_sig: &str,
    ) -> std::result::Result<Arc<TagRetrievalObservation>, String> {
        ...
        let entry = cache
            .get(handle)
            .ok_or_else(|| format!("tag retrieval observation handle {} is unavailable", handle))?;
        if now.duration_since(entry.inserted_at) > self.query_cache_ttl {
            cache.remove(handle);
            return Err(format!(
                "tag retrieval observation handle {} expired",
                handle
            ));
        }
        ...
        {
            return Err("tag retrieval observation artifact generation mismatch".to_string());
        }
    ...
    pub(crate) fn clear(&self) -> std::result::Result<(), String> {
        let mut guard = self
            .active_artifact
            .write()
            .map_err(|error| format!("tag retrieval runtime clear lock failed: {}", error))?;
        *guard = None;
        drop(guard);
        self.clear_query_cache()
    }
    ...
    pub(crate) fn diagnostics(
        ...
        let guard = self
            .active_artifact
            .read()
            .map_err(|error| format!("tag retrieval runtime diagnostics lock failed: {}", error))?;
...
fn decode_artifact(
    db_path: &str,
    artifact_sig: &str,
) -> std::result::Result<Arc<TagGraphArtifact>, String> {
    ...
    let (codec, expected_checksum, compressed): (String, String, Vec<u8>) = connection
        .query_row(
            "SELECT payload_codec, payload_checksum, payload FROM tag_graph_artifacts \
        ...
        .map_err(|error| {
            format!(
                "Propagation structure artifact {} unavailable: {}",
                artifact_sig, error
            )
        })?;
    if codec != "gzip-json-v1" {
        return Err(format!(
            "unsupported PropagationStructure artifact codec: {}",
            codec
        ));
    }
    ...
    limited
        .read_to_end(&mut raw)
        .map_err(|error| format!("decompress PropagationStructure artifact failed: {}", error))?;
    if raw.len() as u64 > MAX_DECOMPRESSED_ARTIFACT_BYTES {
        return Err(format!(
            "PropagationStructure artifact {} exceeds decompressed size limit of {} bytes",
    ...
    if expected_checksum.is_empty() || actual_checksum != expected_checksum {
        return Err(format!(
            "PropagationStructure artifact {} checksum mismatch",
            artifact_sig
        ));
    }
    ...
    let payload: Value = serde_json::from_slice(&raw).map_err(|error| {
        format!(
            "decode PropagationStructure artifact JSON failed: {}",
            error
        )
    })?;
    ...
    let transport = payload
        .get("sharedTransport")
        .ok_or_else(|| "PropagationStructure artifact has no sharedTransport".to_string())?;
    ...
    {
        return Err(format!(
            "PropagationStructure artifact {} contains invalid or duplicate node IDs",
            artifact_sig
        ));
    }
    if row_offsets_u64.len() != node_ids.len() + 1 {
        return Err(format!(
            "PropagationStructure artifact {} CSR rowOffsets length mismatch",
            artifact_sig
        ));
    }
    ...
    {
        return Err(format!(
            "PropagationStructure artifact {} CSR rowOffsets are not monotonic from zero",
            artifact_sig
        ));
    }
    ...
    {
        return Err(format!(
            "PropagationStructure artifact {} CSR edge array length mismatch",
            artifact_sig
        ));
    }
    if targets_u64
    ...
    {
        return Err(format!(
            "PropagationStructure artifact {} CSR target index out of bounds",
            artifact_sig
        ));
    }
    if weights
    ...
    {
        return Err(format!(
            "PropagationStructure artifact {} contains invalid edge weights",
            artifact_sig
        ));
    }
    ...
    let mut shortcut_edges = HashSet::new();
    if let Some(entries) = payload.get("shortcutEdges").and_then(Value::as_array) {
        for entry in entries {
            ...
            let ids: Vec<i64> = key
                ...
                .collect();
            if ids.len() == 2 {
                shortcut_edges.insert((ids[0], ids[1]));
            }
        }
    }
...
fn load_candidates(
    ...
    let mut tag_stmt = connection
        ...
        .map_err(|error| format!("prepare tag candidate_record projection failed: {}", error))?;
    ...
    for candidate in candidates {
        ...
        let rows = tag_stmt
            ...
            .map_err(|error| format!("query candidate tag candidate_record failed: {}", error))?;
...
fn compute_anchor_scores(candidate_records: &mut [CandidateRecord], local_support: &HashSet<i64>) {
    if local_support.is_empty() {
        return;
    }
    let max_hits = candidate_records
        .iter()
        .map(|candidate_record| {
            candidate_record
                .tags
                .iter()
                .filter(|tag| local_support.contains(&tag.id))
                .count()
        })
        .max()
        .unwrap_or(0);
    ...
    for candidate_record in candidate_records {
        let hits = candidate_record
            .tags
            .iter()
            .filter(|tag| local_support.contains(&tag.id))
            .count();
        candidate_record.anchor_score = candidate_record
            .anchor_score
            .max(hits as f64 / max_hits as f64);
    }
}

fn source_top(
    candidate_records: &[CandidateRecord],
    distribution: fn(&CandidateRecord) -> f64,
    limit: usize,
) -> Vec<(i64, f64, usize)> {
    let mut ranked: Vec<(i64, f64)> = candidate_records
        .iter()
        .map(|candidate_record| (candidate_record.id, distribution(candidate_record)))
...
fn select_superset(
    ...
    let sources = vec![
        (
            ...
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.query_score,
                config.query_k,
            ),
        ),
        (
            "denoised_distribution_knn",
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.denoised_score,
                config.denoised_k,
            ),
        ),
        (
            ...
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.local_score,
                config.local_distribution_k,
            ),
        ),
        (
            "extended_support_knn",
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.transfer_score,
                config.extended_distribution_k,
            ),
        ),
        (
            "bm25",
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.bm25_score,
                config.bm25_k,
            ),
        ),
        (
            ...
            source_top(
                &candidate_records,
                |candidate_record| candidate_record.anchor_score,
                config.anchor_k,
            ),
        ),
    ];
    ...
    let mut selected: Vec<CandidateRecord> = candidate_records
        .into_iter()
        .filter_map(|mut candidate_record| {
            let entries = source_map.get(&candidate_record.id)?;
            ...
            candidate_record.union_score = clamp01(
                0.5 * maximum + 0.25 * mean + 0.25 * clamp01(reciprocal * 20.0) + multi_bonus,
            );
            candidate_record.sources = entries.iter().map(|entry| entry.0.clone()).collect();
    ...
    for (index, candidate_record) in selected.iter_mut().enumerate() {
        candidate_record.union_rank = index + 1;
    }
    selected
}

#[derive(Clone)]
struct DistributionWorkspace {
    local: HashMap<i64, f64>,
    transfer: HashMap<i64, f64>,
    local_support: HashSet<i64>,
    extended_support: HashSet<i64>,
    source_ids: HashSet<i64>,
}

fn normalize_distribution(entries: &[(i64, f64)]) -> HashMap<i64, f64> {
    let maximum = entries
        .iter()
...
fn evaluate_path(
    ...
    let mut output = StructurePathOutput::default();
    let mut quality_mass = 0.0;
    for pair in candidate_record.tags.windows(2) {
        let current = &pair[0];
        let next = &pair[1];
        ...
        let distribution_continuity = (local_potential.max(transfer_potential)
            * workspace
                .local
                .get(&next.id)
                .copied()
        ...
        .sqrt();
        let continuity = clamp01(0.5 * semantic_continuity + 0.5 * distribution_continuity);
        let local_supported = workspace.local_support.contains(&current.id)
            && workspace.local_support.contains(&next.id);
        let transfer_supported = workspace.extended_support.contains(&current.id)
            && workspace.extended_support.contains(&next.id);
    ...
    } else if let Some(tag) = candidate_record.tags.first() {
    ...
    output.tag_closure = if candidate_record.tags.is_empty() {
        0.0
    } else {
        candidate_record
            .tags
            ...
            .sum::<f64>()
            / candidate_record.tags.len() as f64
    };
...
#[derive(Clone, Default, Serialize)]
#[serde(rename_all = "camelCase")]
struct PropagationStructureGraphOutput {
    score: f64,
    reliability: f64,
...
fn evaluate_association_graph(
    candidate_record: &CandidateRecord,
    input: &NativeInput,
    artifact: &TagGraphArtifact,
    query_tag_vectors: &HashMap<i64, Vec<f32>>,
) -> PropagationStructureGraphOutput {
    let mut output = PropagationStructureGraphOutput {
        query_nodes: input.query_state.propagation_nodes.len(),
        ..PropagationStructureGraphOutput::default()
    };
    ...
    for node in &input.query_state.propagation_nodes {
        let weight = positive(if node.normalized_activation != 0.0 {
        ...
        let alignment = if let Some((index, tag)) = exact {
            ...
            candidate_record
                ...
                .filter_map(|(index, tag)| {
                    // 查询 trace 与候选路径的原始 Tag 向量均已加载；直接计算精确
                    // 余弦，避免在 TagGraphArtifact 中再常驻一份字符串键 Pairwise 表。
    ...
    let maximum_hop = input
        ...
        .unwrap_or(1)
        .max(1) as f64;
    let propagation_node_by_id: HashMap<i64, &PropagationNode> = input
        .query_state
        .propagation_nodes
        .iter()
        .map(|node| (node.id, node))
        .collect();
    let minimum_position = candidate_record
        .tags
        .first()
        .map(|tag| tag.position)
        .unwrap_or(0);
    let maximum_position = candidate_record
        .tags
        .last()
        .map(|tag| tag.position)
        .unwrap_or(minimum_position);
    let candidate_span = (maximum_position - minimum_position).max(1) as f64;
    let retained_edges: Vec<&PropagationEdge> = input
        .query_state
        .propagation_edges
        .iter()
        .filter(|edge| {
            clamp01(if edge.normalized_flow != 0.0 {
                ...
                edge.flow
            }) >= input.config.minimum_propagation_edge_flow
        })
        .take(input.config.maximum_propagation_edges.max(1))
        .collect();
    ...
    for edge in retained_edges {
        ...
        let source_hop = propagation_node_by_id
            .get(&edge.source_id)
            .map(|node| node.hop)
            .unwrap_or(0);
        let target_hop = propagation_node_by_id
            .get(&edge.target_id)
            .map(|node| node.hop)
            .unwrap_or(0);
        ...
        let independent =
            artifact.independent_fraction(edge.source_id, edge.target_id, candidate_record.file_id);
        ...
        association_mass += edge_weight * edge_quality;
    ...
    if matched_edge_weight > 0.0 {
        ...
        output.edge_association_score = clamp01(association_mass / matched_edge_weight);
    }
    output.motif_score = output.edge_association_score;
    output.edge_graph_score = clamp01(
        0.18 * output.node_alignment_score
            ...
            + 0.28 * output.edge_association_score
    ...
    if output.matched_edges > 0 {
        ...
        output.reliability_mode = "edge_association".to_string();
...
fn evaluate_observables(
    ...
    let chain_size = candidate_record.tags.len().max(1) as f64;
    ...
    for tag in &candidate_record.tags {
        ...
        local_contacts += usize::from(workspace.local_support.contains(&tag.id));
        transfer_contacts += usize::from(workspace.extended_support.contains(&tag.id));
        local_potential += local;
        transfer_potential += transfer;
        tail_only += usize::from(
            (local > 0.0 || transfer > 0.0)
                && !workspace.local_support.contains(&tag.id)
                && !workspace.extended_support.contains(&tag.id),
        );
        boundary = boundary
    ...
    let query_chunk = clamp01(cosine(&input.query.vector, &candidate_record.chunk_vector));
    ...
    let closure = clamp01(0.65 * query_chunk + 0.35 * structure_path.tag_closure);
    ObservableOutput {
        direct,
        structural: structure_path.path_quality,
        thematic,
        closure,
...
fn anchor_contacts(
    ...
    for (seed_id, mass) in seeds {
        if let Some(tag) = candidate_record.tags.iter().find(|tag| tag.id == *seed_id) {
        ...
        if let Some((tag, similarity)) = candidate_record
            .tags
            .iter()
...
fn compute_anchors(
    ...
    let contacts: Vec<Vec<(i64, i64, bool, f64, f64, f64)>> = candidate_records
        .par_iter()
        .map(|candidate_record| anchor_contacts(candidate_record, seeds, seed_vectors, config))
        .collect();
            ...
            for contact in &candidate_contacts {
                ...
                let rarity = config.rarity_floor.max(
                    1.0 - pool_counts.get(&contact.0).copied().unwrap_or(0) as f64
                        / candidate_records.len().max(1) as f64,
                );
...
struct ScoredWork {
    candidate_record: CandidateRecord,
    structure_path: StructurePathOutput,
    association_graph: PropagationStructureGraphOutput,
...
struct QueryMorphology {
    ...
    effective_depth: f64,
    depth_variance: f64,
    activation_concentration: f64,
    shallow_activation_ratio: f64,
...
fn compute_query_morphology(input: &NativeInput) -> QueryMorphology {
    let nodes = &input.query_state.propagation_nodes;
    let edges = &input.query_state.propagation_edges;
    ...
    let activations: Vec<f64> = nodes
        .iter()
        .map(|node| {
            positive(if node.normalized_activation != 0.0 {
                node.normalized_activation
            } else {
                node.activation
            })
        ...
        .collect();
    let total_activation: f64 = activations.iter().sum();
    let weighted_hop = if total_activation > 1e-12 {
        nodes
            .iter()
            .zip(&activations)
            .map(|(node, activation)| node.hop.max(0) as f64 * activation)
            .sum::<f64>()
            / total_activation
    } else {
        0.0
    };
    let hop_variance = if total_activation > 1e-12 {
        nodes
            .iter()
            .zip(&activations)
            .map(|(node, activation)| activation * (node.hop.max(0) as f64 - weighted_hop).powi(2))
            .sum::<f64>()
            / total_activation
    } else {
        0.0
    ...
    let shallow_activation_ratio = if total_activation > 1e-12 {
        nodes
            .iter()
            .zip(&activations)
    ...
    let raw_hhi = if total_activation > 1e-12 {
        activations
            .iter()
            .map(|activation| (activation / total_activation).powi(2))
            .sum::<f64>()
    } else {
        1.0
    };
    let uniform_hhi = 1.0 / node_count.max(1) as f64;
    let activation_concentration = if node_count > 1 {
    ...
    let atomic_logit = 1.45 * shallow_activation_ratio + 0.9 * activation_concentration
    ...
    let association_weights = exponentials.map(|value| value / exponential_sum.max(1e-12));
    let prior = [1.0 / 3.0; 3];
    let weights = [
        confidence * association_weights[0] + (1.0 - confidence) * prior[0],
        confidence * association_weights[1] + (1.0 - confidence) * prior[1],
        confidence * association_weights[2] + (1.0 - confidence) * prior[2],
    ];
...
fn compute_spread(input: &NativeInput) -> SpreadOutput {
    let active_edges = input.query_state.propagation_edges.len();
    let seed_nodes = input
        ...
        .filter(|node| node.hop == 0)
        .count();
    let reached_nodes = input.query_state.propagation_nodes.len();
    let derived_nodes = reached_nodes.saturating_sub(seed_nodes);
    let safe_seed = seed_nodes.max(1) as f64;
    let edge_spread = clamp01(active_edges as f64 / (input.config.kappa_edge * safe_seed));
    let derived_spread = clamp01(derived_nodes as f64 / (input.config.kappa_ratio * safe_seed));
    let flows: Vec<f64> = input
        ...
        .collect();
    let flow_spread = if flows.is_empty() {
        0.0
    } else if flows.len() == 1 {
        0.5
    } else {
    ...
    let combined_spread = (edge_spread.max(input.config.spread_epsilon)
        * derived_spread.max(input.config.spread_epsilon)
        * flow_spread.max(input.config.spread_epsilon))
    .cbrt();
    ...
    let spread = clamp01(combined_spread * observation_factor);
    let spread_class = if spread < input.config.inactive_threshold {
        "inactive"
    } else if spread < input.config.sparse_threshold {
        "sparse"
    } else {
        "broad"
    };
    SpreadOutput {
        ...
        derived_spread,
        flow_spread,
        spread_class: spread_class.to_string(),
        active_edges,
        seed_nodes,
        reached_nodes,
        derived_nodes,
...
fn assign_structure_scores(
    ...
    for item in work.iter_mut() {
        ...
        let direct_answer = mode != "atomic"
            ...
            && (item.direct_evidence >= 0.55
                || (near_frontier && item.candidate_record.query_score >= 0.55));
        let structural = (item.association_graph.matched_edge_coverage
            * item.association_graph.reliability
    ...
    for index in 0..work.len() {
        ...
        let candidate_confidence = if mode == "atomic" {
            (work[index].observables.closure
                * clamp01(
                    0.55 * work[index].association_graph.matched_node_coverage
                        + 0.45 * work[index].association_graph.node_alignment_score,
                ))
            .sqrt()
        } else {
            (work[index].observables.closure
                * clamp01(
                    0.75 * work[index].association_graph.matched_edge_coverage
                        + 0.25 * work[index].association_graph.matched_node_coverage,
                )
                * clamp01(
                    0.7 * work[index].association_graph.reliability
                        + 0.3 * work[index].association_graph.node_alignment_score,
                ))
            .cbrt()
        };
        ...
        let (role_cap, multiplier) = match work[index].role.as_str() {
            "atomic_concept" => (config.structure_bonus_cap, 1.0),
        ...
        let mut bonus = requested.min(role_cap).min(config.structure_bonus_cap);
        ...
        work[index].structure_bonus = clamp01(bonus);
    }
    ...
    let promote = strongest
        ...
        .unwrap_or(false);
    let graph_gate = spread.spread_score.powf(config.spread_gamma.max(0.0));
    ...
    for (index, item) in work.iter_mut().enumerate() {
        ...
        } else if spread.spread_score < config.structure_role_min_spread
            && item.role == "structural_explanation"
        {
            item.role = "thematic_neighbor".to_string();
        }
        item.propagation_bonus = item.structure_bonus * graph_gate;
        item.final_score = clamp01(item.pure_score + item.propagation_bonus + item.anchor_bonus);
    }
}
...
struct NativeResultItem {
    ...
    propagation_trace: Option<Value>,
    #[serde(skip_serializing_if = "Option::is_none")]
    propagation_graph: Option<PropagationStructureGraphOutput>,
    #[serde(skip_serializing_if = "Option::is_none")]
    structure_path: Option<StructurePathOutput>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
struct NativeOutput {
    ...
    artifact_sig: String,
    query_id: Option<String>,
    propagation_spread: SpreadOutput,
    query_mode: String,
...
fn run_native(
    runtime: Option<&TagRetrievalRuntime>,
    db_path: &str,
    artifact_sig: &str,
    input_json: &str,
) -> std::result::Result<String, String> {
    ...
    let mut input: NativeInput = serde_json::from_str(input_json)
        .map_err(|error| format!("invalid PropagationStructure native input JSON: {}", error))?;
    if let (Some(runtime), Some(handle)) = (runtime, input.observation_handle.as_deref()) {
        ...
        input.query_state.local_support_ids = cached.local_support_ids.as_ref().clone();
        input.query_state.extended_support_ids = cached.extended_support_ids.as_ref().clone();
        input.query_state.propagation_nodes = cached
            .observation
            .nodes
            .iter()
            .map(|node| PropagationNode {
                id: node.id,
            ...
            .collect();
        input.query_state.propagation_edges = cached
            .observation
            .edges
            .iter()
            .map(|edge| PropagationEdge {
            ...
            .collect();
        input.query_state.propagation_provenance = cached
            .observation
            .nodes
            .iter()
            ...
            .collect();
        input.query_state.complete_observation = !cached.observation.seed_distribution.is_empty();
    ...
    let runtime = runtime.ok_or_else(|| {
        "PropagationStructure reranker requires the VexusIndex-owned tag-retrieval runtime"
            .to_string()
    })?;
    let artifact = load_artifact_from_runtime(runtime, db_path, artifact_sig)?;
    let mut candidate_records = load_candidates(db_path, &input.candidates, dimension)?;
    let original_score_by_id: HashMap<i64, f64> = input
        ...
        .collect();
    let local_support: HashSet<i64> = input
        .query_state
        .local_support_ids
        .iter()
        .copied()
        .collect();
    compute_anchor_scores(&mut candidate_records, &local_support);
    candidate_records
        .par_iter_mut()
        .for_each(|candidate_record| {
            candidate_record.query_score =
                cosine(&input.query.vector, &candidate_record.chunk_vector);
            candidate_record.denoised_score =
                cosine(&input.denoised_vector, &candidate_record.chunk_vector);
    ...
    let selected_count = candidate_records.len();

    let workspace = DistributionWorkspace {
        local: normalize_distribution(&input.query_state.local_distribution),
        transfer: normalize_distribution(&input.query_state.extended_distribution),
        local_support,
        extended_support: input
            .query_state
    ...
    let connection = open_readonly(db_path)?;
    let propagation_node_ids: Vec<i64> = input
        .query_state
        .propagation_nodes
        .iter()
        .map(|node| node.id)
    ...
    if !propagation_node_ids.is_empty() {
        let mut statement = connection
            .prepare("SELECT vector FROM tags WHERE id = ?1")
            .map_err(|error| format!("prepare query propagation vector read failed: {}", error))?;
        for id in propagation_node_ids {
            if let Ok(bytes) =
    ...
    // before consuming `candidate_records`. The indexed parallel result preserves candidate_record order.
    ...
    let core_tag_names: HashSet<String> = input
        ...
        .filter_map(|entry| {
            candidate_records
                .iter()
                .flat_map(|candidate_record| candidate_record.tags.iter())
                .find(|tag| tag.id == entry.id)
                .map(|tag| tag.name.to_lowercase())
        })
        .collect();

    let mut work: Vec<ScoredWork> = candidate_records
        .into_par_iter()
        .zip(anchor_results.into_par_iter())
        .map(|(candidate_record, anchor)| {
            let structure_path =
                evaluate_path(&candidate_record, &workspace, &artifact, &input.config);
            let association_graph = evaluate_association_graph(
                &candidate_record,
                &input,
                &artifact,
                &query_tag_vectors,
            );
            let visible = !explicit_scope || allowed.contains(&candidate_record.file_id);
            let observables = evaluate_observables(
                &candidate_record,
                &structure_path,
                &input,
                &workspace,
            ...
            let semantic_base = clamp01(
                (input.config.pure_query_weight * clamp01(candidate_record.query_score)
                    + input.config.pure_local_weight * clamp01(candidate_record.local_score)
                    + input.config.pure_transfer_weight * clamp01(candidate_record.transfer_score))
                    / semantic_total,
            );
            let association_score = clamp01(
                0.625 * structure_path.path_quality
                    + 0.375
                        * clamp01(
            ...
            let path_reliability = clamp01(
                structure_path.path_quality / input.config.association_path_saturation.max(1e-6),
            );
            let association_reliability = (path_reliability * observables.query_chunk_score).sqrt();
            let structure_bonus =
                input.config.structure_bonus_cap * association_score * association_reliability;
            let pure_score =
                clamp01(semantic_base + structure_bonus.min(input.config.structure_bonus_cap));
            let atomic_score = 0.75 * association_graph.node_graph_score
                + 0.25 * association_graph.edge_graph_score;
            let propositional_score = 0.25 * association_graph.node_graph_score
                + 0.75 * association_graph.edge_graph_score;
            let narrative_score = 0.15 * association_graph.node_graph_score
                + 0.85 * association_graph.edge_graph_score;
    ...
    let spread = compute_spread(&input);
    let mode = morphology.dominant_mode.as_str();
    assign_structure_scores(&mut work, mode, &spread, &input.config);
        ...
        right
            ...
            .then_with(|| {
                right
                    .candidate_record
                    .union_score
                    .partial_cmp(&left.candidate_record.union_score)
                    .unwrap_or(Ordering::Equal)
            })
            .then_with(|| {
                left.candidate_record
                    .union_rank
                    .cmp(&right.candidate_record.union_rank)
            })
    });
    ...
    let results: Vec<NativeResultItem> = work
        ...
        .map(|(index, item)| {
            ...
            let core_tags_matched = matched_tags
                ...
                .cloned()
                .collect();
            let propagation_trace = if input.include_trace {
                Some(serde_json::json!({
                    "mode": "propagation_structure_observability_gated__with_direct_anchor",
                    "spreadScore": spread.spread_score,
                    "spreadClass": spread.spread_class,
                    "spreadEdge": spread.edge_spread,
                    "spreadDerived": spread.derived_spread,
                    "spreadFlow": spread.flow_spread,
                    "spreadGamma": input.config.spread_gamma,
                    "propagationGate": spread
                        .spread_score
                        .powf(input.config.spread_gamma.max(0.0)),
                    "structureBonus": item.structure_bonus,
                    "propagationBonus": item.propagation_bonus,
            ...
            NativeResultItem {
                id: item.candidate_record.id,
                chunk_id: item.candidate_record.id,
                rank: index + 1,
                score: item.final_score,
                base_score: item.pure_score,
                structure_bonus: item.propagation_bonus,
                anchor_bonus: item.anchor_bonus,
                role: item.role,
                spread_score: spread.spread_score,
                spread_class: spread.spread_class.clone(),
                matched_tags,
                core_tags_matched,
                candidate_sources: item.candidate_record.sources,
                original_score: original_score_by_id
                    .get(&item.candidate_record.id)
                    .copied()
                    .unwrap_or(0.0),
                propagation_trace,
                propagation_graph: input.include_trace.then_some(item.association_graph),
                structure_path: input.include_trace.then_some(item.structure_path),
    ...
    serde_json::to_string(&output).map_err(|error| {
        format!(
            "encode PropagationStructure native output failed: {}",
            error
        )
    })
}

pub struct PropagationStructureRerankerTask {
    runtime: Option<Arc<TagRetrievalRuntime>>,
    db_path: String,
    artifact_sig: String,
    input_json: String,
}

impl Task for PropagationStructureRerankerTask {
    type Output = String;
    type JsValue = String;
...
pub(crate) fn rerank_with_runtime(
    ...
    artifact_sig: String,
    input_json: String,
) -> AsyncTask<PropagationStructureRerankerTask> {
    AsyncTask::new(PropagationStructureRerankerTask {
