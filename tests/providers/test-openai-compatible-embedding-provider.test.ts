...
test("constructor stores config correctly", () => {
  ...
  assert.deepStrictEqual(provider.fallbackModels, ["backup-model", "fallback-model"]);
...
test("constructor parses string fallbackModels into array", () => {
  ...
  assert.deepStrictEqual(provider.fallbackModels, ["model-a", "model-b", "model-c"]);
});
...
test("embedBatch returns empty array for empty input", async () => {
  const provider = new OpenAICompatibleEmbeddingProvider({ model: "test" });
...
test("embedBatch returns nulls for all-oversize texts without calling fetch", async () => {
  ...
  global.fetch = (async (_url: string | URL | Request, _opts?: RequestInit) => {
    fetchCalled = true;
  ...
  try {
    ...
    assert.strictEqual(
      fetchCalled,
      false,
      "fetch should not be called for all-oversize texts",
    );
  } finally {
...
test("embed uses embedBatch under the hood", async () => {
  ...
  let called = false;
  provider.embedBatch = async (texts: readonly string[] | null | undefined) => {
    called = true;
