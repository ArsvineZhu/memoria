...
test("normalizeSource throws TAG_RETRIEVAL_EMPTY_SOURCE on no positive mass", () => {
  const operator = buildRowOperator(makeLineGraph(4));
  assert.throws(
    () =>
      normalizeSource(
    ...
    (error: unknown) =>
      ...
      "code" in error &&
      error.code === "TAG_RETRIEVAL_EMPTY_SOURCE",
  );
});
...
test("solveGraphDiffusion converges local and transfer distributions on a line graph", () => {
  const operator = buildRowOperator(makeLineGraph(5));
  const source = new Map([[3, 1]]);
  const result = solveGraphDiffusion({
    operator,
    seedDistribution: source,
  });

  assert.ok(result.diagnostics.converged, "both distributions should converge");
  ...
  const localIds = result.localSupport.ids;
  assert.ok(localIds.includes(sourceId), "source node in local support");
  assert.strictEqual(
    result.extendedSupport.size >= result.localSupport.size,
    true,
...
test("solveGraphDiffusion propagates mass away from the source", () => {
  const operator = buildRowOperator(makeLineGraph(7));
  const result = solveGraphDiffusion({
    operator,
    seedDistribution: new Map([[4, 1]]),
  });
...
test("solveGraphDiffusion rejects mismatched operator spaces", () => {
  ...
  assert.throws(
    () =>
      solveGraphDiffusion({
        ...
        seedDistribution: new Map([[1, 1]]),
      }),
    /must share the same node space/,
  );
});

test("solveGraphDiffusion throws without operators", () => {
  assert.throws(
    () => solveGraphDiffusion({ seedDistribution: new Map([[1, 1]]) }),
