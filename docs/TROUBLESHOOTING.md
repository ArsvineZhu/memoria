...
| 3   | `corepack pnpm test` 中实时集成测试输出 SKIP                                         | 缺少完整 `EMBED_*` 配置；实时测试会主动跳过                                                                | 需要实时调用时，把兼容服务配置写入 `tutorials/08-provider-selection/.env`（勿提交）；缺少必要配置时跳过是预期行为                                                                                           |
| 4   | 搜索空结果（历史库 `files.path` 为 `spaceX\a.md`，而引用方查 `spaceX/a.md`，命中 0） | Windows `path.relative` 产出反斜杠相对路径，旧库/外部写入未 posix 化                                       | 统一正斜杠：当前入库路径已 `relPath.split(path.sep).join('/')`（file-reader.ts:69）；存量库须把 files.path 转 `a/b.md` 形式或重灌                                                                           |
...
| 8   | provider 选择教程没有调用配置的兼容服务                                              | 配置不完整时按设计使用 fake；fake 只保证流程可运行                                                         | 按 [provider 选择教程](../tutorials/08-provider-selection/README.md) 配置完整 `EMBED_*`/`RERANK_*`，执行 `corepack pnpm tutorial:08`；请求失败会原样报告，不会再切换 fake                                   |
...
- **处理**：OpenAI-compatible provider 的类默认维度 1024 与引擎默认
...
- **牵涉文件**：`tests/integration/real-openai-compatible.test.ts`。其中的实时用例会
  调用配置的 embedding 服务，因此数量和覆盖范围应以当前测试文件为准。
- **机制**：测试从进程环境或 `tutorials/08-provider-selection/.env` 读取
  `EMBED_API_KEY`、`EMBED_API_URL` 和 `EMBED_MODEL`；缺少任一值时以 skip 跳过。
- **正确处置**：本地无完整配置时这是**预期行为**（不是失败）；要调用兼容服务，
  给教程 `.env` 写兼容服务配置（勿提交），并让 `EMBED_DIMENSION` 与服务返回维度一致。

## 4. 搜索空结果 / `files.path` 反斜杠

- **表现**：`engine.search()` 返回 0 条或缺档；直查
  `SELECT path FROM files` 发现 `spaceX\a.md` 样式。
...
test-ingestion-stages.test.ts:132–144` 验证 Windows 风格入参输出
  `space/archived.md`）。**存量库**：要么 SQL 批量把 `path` 里的 `\` 转为
...
- **现状**：仓库内置 win32-x64-msvc、linux-x64-gnu/musl、linux-arm64-
  ...
  target），产物放回包内
  `vexus-lite.<platform>-<arch>.node` 即可在本地 require。

## 8. 记忆召回演示无输出 / 配置缺失

- **行为**：编译后的 `dist-test/tutorials/08-provider-selection/main.js` 根据完整的
  `EMBED_*`/`RERANK_*` 配置选择兼容 provider，否则选择 fake；不会因为请求失败
  再次切换 fake。
- **解法**：在 `tutorials/08-provider-selection/.env` 放完整配置，或设置进程环境变量，
  再执行 `corepack pnpm tutorial:08`。测试和教程都使用 OpenAI-compatible 协议；
  fake 只保证流程可运行，不代表召回质量。
- 若 embedding key 存在仍失败，对照第 1/2 条（目录权限、维度与模型是否一致）；
  external rerank 还必须配置完整的 `RERANK_API_URL`、`RERANK_API_KEY`、
  `RERANK_MODEL`，外部服务失败会原样报告。
