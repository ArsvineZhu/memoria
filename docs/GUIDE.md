...
const engine = createMemoryEngine({
  config: {
    ...
    rootPath: path.join(__dirname, 'notes'),       // 相对路径基准（扫描目录）
    storePath: path.join(__dirname, 'indices'),    // 向量索引延迟落盘目录
    topK: 3
  },
  dbPath: path.join(__dirname, 'memory.sqlite'),   // SQLite 元数据（默认 ':memory:'）
  embeddingProvider: new FakeEmbeddingProvider(128) // 离线确定性伪嵌入
});

(async () => {
  await engine.initialize();                        // 加载 rag 参数、置就绪
  await engine.flushBatch([{ path: path.join(__dirname, 'notes', 'life', 'coffee.md') }]);
