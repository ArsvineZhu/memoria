...
if (API_KEY) {
  console.log(
    `[real-dashscope] Live API key loaded (masked: ${API_KEY.slice(0, 4)}***)`,
  );
} else {
...
test(
  "provider: real batch embedding (document + query, dimension, alignment)",
  skipOpts,
  async () => {
    ...
    for (let i = 1; i < 4; i++) {
      let same = true;
      for (let j = 0; j < DIM; j++) {
        if (Math.abs(docVectors[0]![j] - docVectors[i]![j]) > 1e-9) {
          same = false;
          break;
        }
      }
    ...
    const queryVec = await provider.embedBatch(["量子计算 叠加"], {
      textType: "query",
    });
    ...
    const defaults = await provider.embedBatch([
      "第一句测试文本。",
      "第二句测试文本。",
    ]);
...
test(
  "engine: full MemoryEngine + adapter chain on the real API",
  skipOpts,
  async () => {
    ...
    try {
      ...
      assert.ok(
        afterDelete.files < beforeDelete.files,
        "delete must shrink file count",
      );
    } finally {
      await kb.shutdown();
      cleanupDir(rootPath);
...
test(
  "persistence: disk index (lazy-load) + reopen search, real embeddings",
  skipOpts,
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.ok(
          stats2.files >= 2,
          `persisted files must be re-indexed, got ${stats2.files}`,
        );
...
test(
  "tdb: TDBEngine ingest/search + disk persistence on real API",
  skipOpts,
  async () => {
