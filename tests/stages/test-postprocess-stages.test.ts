...
import ResultDeduplicatorStage = require('../../src/stages/postprocess/result-deduplicator');
import ExternalRerankerStage = require('../../src/stages/postprocess/external-reranker');
import TimeDecayStage = require('../../src/stages/postprocess/time-decay');
import TruncatorStage = require('../../src/stages/postprocess/truncator');
import ExpanderStage = require('../../src/stages/postprocess/expander');
...
test('ResultFormatterStage hydrates partial candidates into full result rows', async () => {
  ...
  const tagMemoTrace = { activations: new Map<number, number>(), rankedTags: ['重要'], iterations: 3 };
  const out = await stage.process({
    query: '记忆',
