...
function relativeCandidatePath(candidate: ChunkCandidate): string {
  ...
  let normalized = raw.normalize("NFKC").replace(/\\/g, "/").trim();
...
export function createOpenAICompatibleReranker(
  ...
  if (!apiUrl) throw configurationError("apiUrl is required");
  if (!apiKey) throw configurationError("apiKey is required");
  if (!model) throw configurationError("model is required");
