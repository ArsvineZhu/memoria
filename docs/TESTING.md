...
| `corepack pnpm lint`             | Oxlint 检查 `src`、`tests`、`tutorials`、`scripts` |
...
`tests/integration/real-openai-compatible.test.ts` 从进程环境或
`tutorials/08-provider-selection/.env` 读取 `EMBED_API_KEY`、`EMBED_API_URL`、`EMBED_MODEL`
和可选的 `EMBED_DIMENSION`。缺少任一必要配置时，集成用例会明确跳过；这不是断言
失败。配置完整后，它才会访问 OpenAI-compatible embedding service 并测试持久化等
路径。不要提交 `.env`。

教程使用 `FakeEmbeddingProvider` 或配置的兼容 provider；provider 选择和 fake 边界见
[../tutorials/08-provider-selection/README.md](../tutorials/08-provider-selection/README.md)。

## 如何报告不完整验证

如果缺少原生二进制、Rust 工具链、实时密钥、网络或目标平台产物，应报告具体
命令和限制。TypeScript 构建成功不能证明原生冒烟、provider 网络链路或其他平台行为成功。
