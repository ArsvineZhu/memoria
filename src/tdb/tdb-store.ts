...
class TDBStore implements TdbStoreContract {
  ...
  async getFileById(id: number): Promise<TdbFileRow | null> {
    const row = this.db.prepare('SELECT * FROM files WHERE id = ?').get(id) as TdbFileQueryRow | undefined;
    return row ? this._fileRow(row) : null;
  }
   ...
   */
  async deleteFile(library: string, path: string): Promise<{ chunkIds: number[]; nodeIds: number[] }> {
  ...
  async getChunks(library: string, path: string): Promise<TdbChunkRow[]> {
    ...
    ).all(library, path) as Array<Pick<TdbChunkQueryRow, 'id' | 'chunk_index' | 'node_id' | 'text' | 'checksum'>>;
    return rows.map(r => ({
      library,
      path,
      id: r.id,
  ...
  async getMeta(key: string): Promise<string | null> {
    const row = this.db.prepare('SELECT value FROM tdb_meta WHERE key = ?').get(key) as TdbMetaRow | undefined;
