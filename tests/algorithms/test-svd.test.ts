...
test("computeWeightedPCA extracts principal components", () => {
  ...
  const result = computeWeightedPCA({ vectors, weights, labels }, dim, {
    maxBasisDim: 3,
  });
  ...
  assert.strictEqual(
    at(result.U, 0, "PCA basis").length,
    dim,
  ...
  for (let i = 1; i < result.S.length; i++) {
    assert.ok(
      at(result.S, i, "eigenvalues") <= at(result.S, 0, "eigenvalues"),
...
test("selectBasisDimension returns at least 8 when possible", () => {
  ...
  assert.ok(
    k >= 8,
    "should return at least 8 components for well-distributed eigenvalues",
  );
});
